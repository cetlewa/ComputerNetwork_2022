...
隨即訪問：用戶可隨機發送信息，可能產生碰撞(衝突)

​	ALOHA，CSMA，CSMA/CD，CDMA/CA

受控訪問：多點線路探詢或輪詢 -> 局域網中使用較少
...
節點發送數據之前先檢測是否有其他節點在發送，如果有就等待 -> 發送前先聽

##### 碰撞檢測

- 節點邊發送數據邊檢測信道上的信號電壓的變化情況。

- 由於線路的傳播時延單，靠載波監聽不能完全避免碰撞。

- 在發生碰撞時，兩個或更多的信號在總線上相互疊加，導致無法識別

##### 碰撞強化

發送方檢測到碰撞後立刻停止發送，並發送32bit或48bit的人為干擾信號，以便讓用戶都知道已經發生了碰撞

##### 碰撞退避

碰撞後節點等待一段時間，重新開始載波檢測和發送操作

為避免再次碰撞，衝突各方的等待時間應該各不相同
...
退避時間 T = 2 τ * 倍數
	...
	- τ ：端到端單向傳播時延
	- 倍數：在 0, 1, ... , 2<sup>k</sup>-1 中隨機取出一個數， k = Min[重傳次數, 10]

##### 爭用期

一個節點開始發送數據後最多經過 2τ 就可知道是否發生了碰撞

端到端往返時延 2τ 為爭用期/碰撞窗口

經過爭用期為檢測到碰撞，則可以肯定這次發送不會發生碰撞

以太網規定了最短幀長64字節，即512比特。10Mbit/s 的以太網發送 512bit 的時間為 51.2μs，即為爭用期

## 局域網技術標注

局域網的數據鏈路層分為兩個子層：LLC邏輯鏈路控制子層、MAC介質訪問控制子層

## 以太網 Ethernet

### 物理層

總線結構，同軸電纜50Ω，傳輸速率10Mbps，兩端分別安裝匹配電阻

總線上單點故障會導致全網癱瘓，網絡中街點過多時可靠性較差

後發展為星形結構，使用集線器連結各個節點，採用便宜靈活的雙絞線

技術稱為 10Base-T 10Mbps 基帶傳輸 雙絞線

#### 採用曼徹斯特編碼

用電平的調變表示0或1，即每個碼元都有電平跳變

### MAC層

採用CSMA/CD協議

節點發送數據時，以太網總線結構，總線上所有節點都能收到幀

按照IEEE802.3標準，給每個節點分配唯一的MAC地址

MAC地址為48bit，高24bit為廠商標示符，低24位由廠商自行分配，需要保證每個網羅接口具有全球唯一的MAC地址

適配器每收到一個幀就檢查幀中的目的MAC地址，如果是發往本站的幀則進行處理，否則丟棄

### 幀格式

目的地址、源地址：各6字節的MAC地址

類型：2字節，標明上層協議類型，例：0x0800表示IP包

數據：網絡層數據報，長度46～1500字節

FCS：4字節，幀校驗序列，採用CRC校驗
...
仍使用IEEE802.3 CSMA/CD協議

MAC幀格式仍然是802.3標準規定的

最短幀長不變，但將一個網段的最大電纜長度減小到100m
...
Virtual LAN 在現有局域網基礎上，通過將網絡站點分組，構成若干邏輯上獨立的虛擬局域網
